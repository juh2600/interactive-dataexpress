...
const hashStuff = (accountDetails) => {
	...
	if(accountDetails.questions && accountDetails.questions.forEach) // forEach? that was very jank and you should feel bad FIXME
	...
	return Accounts.findOne(
		...
		if (account) {
			...
			for (let q in account.questions) {
				account.questions[q].answer = undefined;
			}
		}
		return account;
	});
...
const checkSecurityQuestion = async (username, question_id, answer) => {
	return Accounts.findOne(
		{ "username": username },
		(err, account) => {
			if (err) throw err;
